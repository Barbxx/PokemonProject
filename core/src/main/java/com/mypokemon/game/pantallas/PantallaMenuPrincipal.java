...
public class PantallaMenuPrincipal extends PantallaBase {
    ...
    String currentSubScreen = null; // null, "PARTIDA", "HELP", "ABOUT" (PARTIDA is now likely PantallaPartidas)
    ...
    public PantallaMenuPrincipal(final PokemonPrincipal game) {
        super(game);
    ...
    public void render(float delta) {
        ...
        if (currentOption != -1 && enter) {
            if (currentOption == 0) { // PLAY -> Go to PantallaEleccionJuego
                game.setScreen(new PantallaEleccionJuego(game));
                dispose();
                return;
            } else if (currentOption == 1) { // CARGAR -> Go to PantallaPartidas
                game.setScreen(new PantallaPartidas(game));
                dispose();
                return;
            } else if (currentOption == 2) {
                currentSubScreen = "HELP";
            } else if (currentOption == 3) { // ABOUT -> Go to PantallaAcercaDe
